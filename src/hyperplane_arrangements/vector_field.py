...
class VectorFieldModule(SageObject):
    ...
    def is_independent(self):
        ...
        return self.check_saito_criterion().det() != 0

    def check_saito_criterion(self):
    ...
    def vector_basis(self):
        ...
        if len(Sk_dic) == 0:
            return VectorFieldModule([])
        V1 = self.flatten_coefficients(Sk_dic, n=n_comp)
        return VectorFieldModule([self.gens[i] for i in V1.pivots()])

    def flatten_coefficients(self, Sk_dic, n=0):
    ...
    def graded_component(self, deg):
        ...
        result = []
        for d, u in zip(self.degrees(), self.gens):
            if d <= deg:
    ...
    def _differential_free(self, op, n_comp):
        ...
        MD = self._flatten_scalars(C, Sk_dic, n_comp) if n_comp == 1 else VectorFieldModule(C).flatten_coefficients(Sk_dic, n=n_comp)
    ...
    def affine_basis(self):
        ...
        if len(self.gens[0]) == n - 1:
            ...
            affine_fields = VectorFieldModule([vector(g.v[:n - 1]) for g in self.dehomogenize().gens])

        return affine_fields.vector_basis()

    def fit(self, A, Obs, verbose=True):
        """Fit vector fields to observations."""
        from .fit import fit_vf as _fit_vf
        return _fit_vf(A, Obs, self, verbose=verbose)

    def fit_vorticity(self, A, Obs, verbose=True):
        """Fit vector fields to vorticity observations."""
        from .fit import fit_vorticity as _fit_vorticity
        return _fit_vorticity(A, Obs, self, verbose=verbose)
